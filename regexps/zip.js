 ...
 * isValue test if the entry has the format of a zip code
 * isKeyValuePair test if both key and value are valid
 *
 */

const keyRe = /^zip|^postal|^code/i,
valueRes = {
  AL: {
    rule: '^[0-9]{1,8}$',
    description: [
...
getIsValue = valueRe => str => str && typeof str === 'string' && valueRe.test(str.trim()),
Zip = {};

for (k in valueRes) {
  const valueRe = new RegExp(valueRes[k].rule), isValue = getIsValue(valueRe);
  Zip[k] = {
    keyRe,
    valueRe,
    isKey,
